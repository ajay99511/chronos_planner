  ...
  group('Schema Migration', () {
    test('v4 to v5 migration', () async {
      ...
      await executor.runCustom(
        ...
        ['d1', 'Feb 10', 'Monday', DateTime(2026, 2, 10).millisecondsSinceEpoch, '2026-W07'],
      );
      ...
      final activeDays = await database.customSelect('SELECT * FROM template_active_days').get();
      expect(activeDays.length, 3);
      final days = activeDays.map((row) => row.read<int>('day_index')).toSet();
      expect(days, {0, 2, 4});

      // Check PlanTemplates columns
      final templateInfo = await database.customSelect('PRAGMA table_info(plan_templates)').get();
      final templateCols = templateInfo.map((c) => c.read<String>('name')).toSet();
      expect(templateCols.contains('active_days'), isFalse);

      // Check DayPlans columns
      final dayPlanInfo = await database.customSelect('PRAGMA table_info(day_plans)').get();
      final dayPlanCols = dayPlanInfo.map((c) => c.read<String>('name')).toSet();
      expect(dayPlanCols.contains('date_str'), isFalse);
      expect(dayPlanCols.contains('day_of_week'), isFalse);

      // Check TodoItems columns
      final todoInfo = await database.customSelect('PRAGMA table_info(todo_items)').get();
      final todoCols = todoInfo.map((c) => c.read<String>('name')).toSet();
      expect(todoCols, containsAll(['item_type', 'duration_minutes', 'checklist_json', 'audio_file_path']));
...
class TestDatabase extends AppDatabase {
  TestDatabase(QueryExecutor executor) : super.forTesting(DatabaseConnection(executor));
