...
class ScheduleView extends StatelessWidget {
                    ...
                    child: AnimatedContainer(
                      ...
                      decoration: BoxDecoration(
                        gradient: isSelected
                            ...
                            colors: [AppColors.surface.withValues(alpha: 0.8), AppColors.surface.withValues(alpha: 0.5)]
                        ),
                        ...
                        boxShadow: isSelected
                            ? [
                          BoxShadow(color: AppColors.neonBlue.withValues(alpha: 0.5), blurRadius: 16, offset: const Offset(0, 8)),
                          BoxShadow(color: AppColors.neonBlue.withValues(alpha: 0.2), blurRadius: 4, offset: const Offset(0, 2))
                        ]
                            : [],
                      ),
                      child: Stack(
                        ...
                        InkWell(
                          onTap: () {
                            showModalBottomSheet(
                                ...
                                builder: (_) => AddTaskSheet(onAdd: (t) => provider.addTask(t))
                            );
                          },
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                                ...
                                boxShadow: [BoxShadow(color: AppColors.neonBlue.withValues(alpha: 0.4), blurRadius: 12, offset: const Offset(0, 4))]
                            ),
                ...
                child: Column(
                  ...
                  children: [
                    Container(
                      ...
                      child: Icon(Icons.calendar_today_outlined, size: 48, color: Colors.white.withValues(alpha: 0.1)),
                    ),
                    const SizedBox(height: 24),
                    Text("No plans for ${dayPlan.dayOfWeek}", style: const TextStyle(color: Colors.white60, fontSize: 16)),
                    const SizedBox(height: 8),
                    TextButton.icon(
                      onPressed: () {
                        // Ideally navigate to templates or show template picker
                        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Go to WorkPlans to apply a template!")));
                      },
                  ...
                  : ListView.builder(
                padding: const EdgeInsets.fromLTRB(20, 0, 20, 100), // Bottom padding for FAB clearance if needed
                physics: const BouncingScrollPhysics(),
                itemCount: sortedTasks.length,
                itemBuilder: (context, index) {
                  return TaskCard(
                    task: sortedTasks[index],
                    onToggle: () => provider.toggleTaskComplete(sortedTasks[index].id),
                    onDelete: () => provider.deleteTask(sortedTasks[index].id),
                  );
                },
              ),
            ),
  ...
  void _showSaveTemplateDialog(BuildContext context, ScheduleProvider provider) {
    if (provider.selectedDay.tasks.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Cannot save an empty day as a template.")));
      return;
    }
